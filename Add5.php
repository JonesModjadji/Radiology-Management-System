...
include("Connection.php");
			...
			$RECEPTIONIST ="";
			$REFDOCTOR ="";
            $Searching1 = "SELECT * FROM study,$Branch WHERE study.SESSIONID = '$SESSIONID1' AND study.PatientsID = '$MRN' AND $Branch.MRN='$MRN'";
...
$ACCESSION = str_pad($PATIENTSID1,10,'0',STR_PAD_LEFT)."$ACCINDEXLetter";
...
elseif (isset($_POST['Home'])) {
	 $sql1 = "UPDATE $Branch SET AccIndex2 ='Done'  WHERE PatientsID = '$PATIENTSID1'";
	  $results2 = mysqli_query($con,$sql1,);
	   if ($results2) {
...
$Searching1 = "SELECT * FROM study WHERE SESSIONID = '$SESSIONID1' AND PatientsID = '$MRN' ";
       $SearchingResults1=mysqli_query($con,$Searching1);
       if($SearchingResults1->num_rows > 0){
  while ($rows=mysqli_fetch_array($SearchingResults1)) {
...
<table>
  ...
  <tr>
        <td><input type="text" name="PATIENSTID" value="<?php echo $PATIENTSID1;?>" style="width: 142px;border-radius: 20px 20px 20px 20px;padding-left: 20px;padding-right: 20px;border: 1px solid white;background: black;color: white;text-align: center;" readonly></td>
...
<form action="" method="post"  enctype="multipart/form-data"><td><div class="hover">
 <button class="input-group-text" type="submit" name="Home" style="color: white;"><i class="fa fa-times"></i> GO TO RECEPTIONIST</button>
  ...
  <tr>
  <td>
  <?php $Searching1 = "SELECT * FROM study WHERE SESSIONID = '$SESSIONID1' AND PatientsID = '$MRN' AND DATE(ExamDate) = '$DATE'";
       $SearchingResults1=mysqli_query($con,$Searching1);
       if($SearchingResults1->num_rows > 0){
