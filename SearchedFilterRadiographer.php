...
$var = ob_get_clean();
       ...
       header("location: SearchedFilterRadiographer.php?MRN=$MRNFILTER&S=$SFILTER&NAME=$NAMEFILTER&SURNAME=$SURNAMEFILTER&PHYSICIAN=$PHYSICIAN&FROM=$FROMFILTER&TO=$TOFILTER".http_build_query($STATUS));
	   }
   }}
   if (isset($_POST['Save'])) {
           $PHYSICIAN3= $_POST['PHYSICIAN3'];
		   $ACCESSION3= $_POST['ACCESSION3'];
$qry=mysqli_query($con,"UPDATE study SET AssignedRadiographer='$PHYSICIAN3'  WHERE Accession = '$ACCESSION3'"); 
...
<title>SearchedRadiographer</title>
...
echo '<option value="'.$TITLE.' '.$INITIALS.' '.$SURNAME.'">'.$INITIALS.' '.$SURNAME.' '.$TITLE.'</option>';
  ...
  ?>
	  ...
	  <tr>
       ...
       <tr>
		...
		<input type="checkbox" id="StartExam" name="STATUS[]" value="Start Exam"><label for="StartExam">Start Exam </label>
		<input type="checkbox" id="Started" name="STATUS[]" value="Started"><label for="Started">Started </label><br>
		<input type="checkbox" id="QCPending" name="STATUS[]" value="QC Pending"><label for="QCPending">QC Pending </label>
		</td>
      </tr>
    </table>
  </div>
  ...
  while ($rows=mysqli_fetch_array($SearchingResults1)) {
	...
	<tr>
	<td style="text-align: center;color: white;">
		<input type="checkbox" id="Processing" name="STATUS[]" value="Processing"><label for="Processing">Processing </label>
 ...
 <input type="hidden" name="SearchFilter" class="input-group-text" id="Search1">
	  ...
	  <td>
	  <div class="hover">
 <button class="input-group-text" onclick="window.open('Radiographer.php');" style="color: white;"><i class="fa fa-times"></i> CLEAR</button>
 </div>
	  </td>
	  </tr>
	  </table>
    ...
    <table border="1px" style="width: 100%;">
       <tr>
		...
		<td style="text-align: center;color: red;">RADIOGRAPHER</td>
...
$var2 = ob_get_clean();
        ...
        $Searching1 = "SELECT * FROM study,$var WHERE study.SESSIONID=$var.SESSIONID AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending')AND $var.Branch LIKE '%$var2%'
       ...
       $Searching2 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending') AND $var.Branch LIKE '%$var2%'
		AND (study.Status ='$STATUS1s' OR study.Status ='$STATUS2' OR study.Status ='$STATUS3' OR study.Status ='$STATUS4' 
		OR study.Status ='$STATUS5' OR study.Status ='$STATUS6' OR study.Status ='$STATUS7' OR study.Status ='$STATUS8' 
		OR study.Status ='$STATUS9' OR study.Status ='$STATUS10' OR study.Status ='$STATUS11' OR study.Status ='$STATUS12'))";
		
		$Searching3 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending')AND $var.Branch LIKE '%$var2%'
		...
		$Searching4 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending') AND $var.Branch LIKE '%$var2%'
		AND ($var.Name='$NAMEFILTER' AND $var.Surname='$SURNAMEFILTER') AND '$STATUS1s'='' )";
		
		$Searching5 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.PatientsID=patients.MRN AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending')  
		...
		$Searching6 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.PatientsID=patients.MRN AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending') 
		AND ($var.Name='$NAMEFILTER' AND $var.Surname='$SURNAMEFILTER') AND '$STATUS1s'='$var2' )";
		
		$Searching7 = "SELECT * FROM study,$var WHERE (study.SESSIONID=$var.SESSIONID AND (study.PatientsID=patients.MRN AND (study.Status ='Start Exam' OR study.Status ='Started' OR study.Status ='Processing' OR study.Status ='QC' OR study.Status ='QC Pending') AND ($var.Branch='$STATUS1s' OR $var.Branch='$STATUS2' OR $var.Branch='$STATUS3'
		 OR $var.Branch='$STATUS4' OR $var.Branch='$STATUS5' OR $var.Branch='$STATUS6' OR $var.Branch='$STATUS7'
		  OR $var.Branch='$STATUS8' OR $var.Branch='$STATUS9' OR $var.Branch='$STATUS10' OR $var.Branch='$STATUS11'
		  OR $var.Branch='$STATUS12' OR $var.Branch='$STATUS13'
		 OR $var.Branch='$STATUS14' OR $var.Branch='$STATUS15' OR $var.Branch='$STATUS16' OR $var.Branch='$STATUS17'
...
if (($NAMEFILTER !=="")&&($SURNAMEFILTER =="")&&($STATUS1s =="")) {
  ...
  while ($rows1=mysqli_fetch_array($SearchingResults3) ) {
	...
	$DOCTOR = $rows1['AssignedRadiographer'];
	$NAME1 = $rows1['Name'];
	...
	$SESSIONID1 = $rows1['SESSIONID'];
     ...
     <tr>
        ...
        <td style="text-align: center;color:white;"><a href="Add3.php?SESSIONID='.$SESSIONID1.'&MRN='.$MRN1.'&PatientsID='.$PATIENTSID1.'&Accession='.$ACCESSION1.'" 
		style="text-decoration:none;color: red;">'.$NAME1.'</a></td>
		<td style="text-align: center;color:white;"><a href="Add3.php?SESSIONID='.$SESSIONID1.'&MRN='.$MRN1.'&PatientsID='.$PATIENTSID1.'&Accession='.$ACCESSION1.'" 
		style="text-decoration:none;color: red;">'.$SURNAME1.'</a></td>
		<td style="text-align: center;color:white;">'.$DOCTOR.'<button style="float:right;" onclick="new_disp(\''.$ACCESSION1.'\');">ADD</button></td>
